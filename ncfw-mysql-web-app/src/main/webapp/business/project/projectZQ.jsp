...
<%
	...
	String mpid = request.getParameter("mpid")==null?"":request.getParameter("mpid");
...
<head>
	...
	<script type="text/javascript" src="<%=basePath%>business/project/MedProject.js"></script>
	...
	var fileMan = new FileManager("fileListPanel",mpid,"t_med_project","view","common");
	//var fileManHt = new FileManager("fileListPanelht",mpid,"t_med_project","view","ht");
    //数据字典
    var DataDict = {
	...
	//初始化页面信息
	function initPage () {
		MedProject.queryMedProjectBypid(pid,mpid);

		//显示意见
		searchMinds(mpid,"t_med_project","minds-div");
	}

	function initPageShow() {
		Ext.getCmp('sborg').setValue(sbDeptName);
		var issp = Ext.getCmp('issp').getValue();
		if((issp == "YES" && openType == "pendingZQ")
				|| openType == "draft") {
			//用户为起草人
			if(user == Ext.getCmp('creater').getValue()) {
				flowBaseInfo.hide();
				Ext.getCmp('pstatus').setValue('ZQJCZ');
			...
			//Ext.getCmp('ht-upload-Button').hide();
			Ext.getCmp('sb_Btn').hide();
		} else {
			flowBaseInfo.hide();
			Ext.getCmp('fileUploadButton').hide();
	...
	/*
	function uploadHt() {
		FileConstant.fileManager = fileManHt;
		FileConstant.showUploadWindow();
	}*/
	   					...
	   					<ext:panel var="mainPanel" layout="border" border="false" otherProperties="split: true" region="center"
	   						...
	   						<ext:items>
								<ext:formPanel var="submitPrjFormPanel" id="submitPrjFormPanel" collapsible="false"
									...
									labelWidth="200" otherProperties="split: true" width="300">
									<ext:items>
										<ext:fieldSet layout="column" defaults="{columnWidth: 1, layout: 'form'}" lazyInit="true" 
											...
											<ext:items>
												...
												<ext:hidden name="medproject.pid" id="_pid"></ext:hidden>
												<ext:hidden name="medproject.mpid" id="mpid"></ext:hidden>
												<ext:hidden name="medproject.issp" id="issp"></ext:hidden>
												
												<ext:panel lazyInit="true">
													<ext:items>
														<ext:textField id="pname" name="medproject.pname"  fieldLabel="项目名称" allowBlank="true" otherProperties="anchor: MedProject.fieldAnchor">
														</ext:textField>
													</ext:items>
												</ext:panel>
												<ext:panel lazyInit="true">
													<ext:items>
														<ext:textField id="cdorg" name="medproject.cdorg" fieldLabel="承担单位<span class='red'>(必填)</span>"  allowBlank="false" otherProperties="anchor: MedProject.fieldAnchor">
														</ext:textField>
													</ext:items>
												</ext:panel>
												...
												<ext:container layout="column" defaults="{width: 465, layout: 'form'}" lazyInit="true">
													<ext:items>
														<ext:panel lazyInit="true">
															<ext:items>
																<ext:textField id="pmanager" name="medproject.pmanager"  width="300" fieldLabel="项目负责人<span class='red'>(必填)</span>"  allowBlank="false" otherProperties="anchor: MedProject.fieldAnchor">
																</ext:textField>
															</ext:items>
														</ext:panel>
														<ext:panel lazyInit="true">
															<ext:items>
																<ext:textField id="tel" name="medproject.tel" width="285" fieldLabel="联系电话<span class='red'>(必填)</span>"  allowBlank="false" otherProperties="anchor: MedProject.fieldAnchor">
																</ext:textField>
															</ext:items>
														</ext:panel>
													</ext:items>
	   						...
	   						<ext:buttons>
								<ext:button id="sb_Btn" text="项目中期检查申报" handler="function() {MedProject.submitMedProject(mpid,'NONSP');}" iconCls="icon_sure" otherProperties="minWidth: Common.config.buttonWidth.defaultButton"></ext:button>
								<ext:button id="sp_ok_Btn" text="审批通过" hidden="true" handler="function() {MedProject.submitMedProject(mpid,'SUCCESS');}" iconCls="icon_sure" otherProperties="minWidth: Common.config.buttonWidth.defaultButton"></ext:button>
								<ext:button id="sp_not_Btn" text="审批不通过" hidden="true" handler="function() {MedProject.submitMedProject(mpid,'FAILURE');}" iconCls="icon_unUse" otherProperties="minWidth: Common.config.buttonWidth.defaultButton"></ext:button>
								<ext:button id="up_pro_Btn" text="修改" hidden="true" handler="function() { MedProject.update();}" iconCls="icon_sure" otherProperties="minWidth: Common.config.buttonWidth.defaultButton"></ext:button>
								<ext:button id="return_Btn" text="返回" hidden="true" handler="function() { MedProject.gotoPending();}" iconCls="icon_close" otherProperties="minWidth: Common.config.buttonWidth.defaultButton"></ext:button>
							</ext:buttons>
	   					</ext:panel>
   					</ext:items>
   				</ext:panel>
