...
<head>
	...
	function initPage () {
		...
		if(!Ext.isEmpty(pid)) {
			...
			//新建
			Project.generateProjectNum("WHBCXJ");
    ...
    <ext:ext onReady="initPage();">
	  	<ext:viewport layout="border" otherProperties="split: true">
	  		...
	  		<ext:tabPanel region="center" border="false" frame="false" otherProperties="activeTab: 0" var="tabPanel">
	  			<ext:items>
  				<ext:panel title="文化部创新奖" layout="border" border="false" iconCls="icon_plugin">
  					<ext:items>
	   					<%-- 列表面板 --%>
									...
									<ext:items>
										<ext:fieldSet layout="column" defaults="{columnWidth: 1, layout: 'form'}" lazyInit="true" 
											...
											<ext:items>
												...
												<ext:hidden name="project.pmtype" id="pmtype" value="WHBCXJ"></ext:hidden>
												...
												<ext:panel lazyInit="true">
													<ext:items>
														<ext:textField id="gjorg" name="project.gjorg" fieldLabel="完成单位<span class='red'>(必填)</span>"  allowBlank="false" otherProperties="anchor: Project.fieldAnchor">
														</ext:textField>
													</ext:items>
												</ext:panel>
												<ext:panel lazyInit="true">
													<ext:items>
														<ext:textField id="tjorg" name="project.tjorg" fieldLabel="推荐单位<span class='red'>(必填)</span>"  allowBlank="false" otherProperties="anchor: Project.fieldAnchor">
														</ext:textField>
													</ext:items>
												</ext:panel>
												<ext:panel lazyInit="true">
