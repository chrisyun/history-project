...
<head>
    ...
    var listUrl   	  	= sys.basePath + "projectAction!searchAll.do";
	var roleId			= '<%=roleId%>';
   	...
   	var onSubmitQueryHandler = function () {
		...
		if(startDate != "" && endDate != ""){
   	 	...
   	 	store.baseParams = {
 			...
 			'curActivityName':Ext.getCmp("query.curActivityName").getValue(),
	...
	function initPage () {
		if(roleId != 'projectsp' || 'ZB' != proprovincecode) {
			Ext.getCmp('project_lx_Btn').hide();
	...
	function onExcel(){
		...
		var startDate = Ext.getCmp("query.sbDateBegin").getValue();
		var endDate = Ext.getCmp("query.sbDateEnd").getValue();
		var curActivityName = Ext.getCmp("query.curActivityName").getValue();
		startDate = (startDate == "") ? "" : startDate.format("Y-m-d");
		endDate = (endDate == "") ? "" : endDate.format("Y-m-d");
		if(startDate != "" && endDate != ""){
			if(startDate > endDate){
		...
		Ext.getDom("excelIframe").src = sys.basePath + 'projectAction!exportProject.do?pname=' 
			+ pname + '&dept=' + dept + '&pmtype=' + pmtype  + '&pstatus=' + pstatus 
			 + '&spstatus=' + spstatus + '&pwtype=' + pwtype + '&curActivityName=' + curActivityName 
			 + '&startDate=' + startDate + '&endDate=' + endDate + '&radom=' + Math.random();
	}
    ...
    <ext:ext onReady="initPage();">
	  	<ext:viewport layout="border" otherProperties="split: true">
	  		...
	  		<ext:tabPanel region="center" border="false" frame="false" otherProperties="activeTab: 0">
	  			<ext:items>
  				<ext:panel title="申报项目查询" layout="border" border="false" iconCls="icon_plugin">
  					<ext:items>
	   					<%-- 列表面板 --%>
	   					<ext:panel var="pendingListPanel" layout="border" border="false" otherProperties="split: true" region="center"
	   						...
	   						<ext:items>
								...
								<ext:formPanel var="queryFormPanel" id="queryFormPanel" title="查询条件" collapsible="true" 
									...
									<ext:items>
										<ext:container layout="column" defaults="{width: 380, layout: 'form'}" lazyInit="true">
											<ext:items>
												...
												<ext:container lazyInit="true">
													<ext:items>
														<ext:comboBox id="query.curActivityName" triggerAction="all"
															fieldLabel='当前环节' allowBlank="true" width="180"
															displayField="text" mode="local" editable="false"
															selectOnFocus="true" forceSelection="true" valueField="value" value=""
															store="new Ext.data.SimpleStore({
																fields : ['value', 'text'],
																data : Common.config.nullArray.concat(DataDict.activityNameArray)
															})">
														</ext:comboBox>
													</ext:items>
												</ext:container>
	   							...
	   							<%-------------------------------------------------------------------------------------------------
									定义数据列表  
								-------------------------------------------------------------------------------------------------%>
								<ext:gridPanel var="dataGrid" id="dataGrid" region="center" iconCls="icon_grid" title="项目列表(<span style='color:blue;'>双击查看详情</span>)"
									border="true" otherProperties="split: true" iconCls="icon_grid" loadMask="true" viewConfig="{forceFit: true}" selModel="sm"
								listeners="{celldblclick : function ( thisGrid, rowIndex, columnIndex, ev ) {Project.showPendingWindow('searchList');}}">
									<ext:tbar>
										<ext:toolbarSeparator></ext:toolbarSeparator>
										<ext:toolbarButton id="project_lx_Btn" text="项目立项" iconCls="icon_sure" handler="Project.updateProjectStatus"
									...
									<ext:columnModel id="cm">
										...
										{header : '申报时间', width: 50, dataIndex: 'creattime',sortable:true,sortable:true}
									</ext:columnModel>
									<%-- 定义列表的分页工具栏--%>
